...
class CommandVideoFlow:
    def __init__(
        self, context: Context, config: PluginConfig, video_service: VideoService
    ):
        self.context = context
        self.cfg = config
        self.video_service = video_service

    async def handle_search_command(self, event: AstrMessageEvent) -> str | None:
    ...
    async def _handle_page_turn(
        ...
        new_videos = await self.video_service.search_video(keyword=keyword, page=page)
        if not new_videos:
